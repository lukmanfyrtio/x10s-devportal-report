...
public class PSQLDashboardServiceImpl implements DashboardService {
	...
	public List<?> getTopTenApiUsage(String filter, String owner, int top, String keyType) throws Exception {
		...
		switch (filter) {
	    case "today":
	        query = "SELECT 'today' as type, TO_CHAR(du.REQUEST_TIMESTAMP, 'YYYY-MM') AS intervalData, du.API_NAME, COUNT(*) AS total_usage " +
	                ...
	                "LEFT JOIN " + this.dbUtilsBilling.getSchemaName() + ".subscription s " +
	                "ON s.subscription_id = du.SUBSCRIPTION_UUID " +
	        ...
	        break;
	    case "week":
	        query = "SELECT 'week' as type, TO_CHAR(du.REQUEST_TIMESTAMP, 'YYYY-MM-DD') AS intervalData, du.API_NAME, COUNT(*) AS total_usage " +
	                "FROM DATA_USAGE_API du " +
	                "INNER JOIN (SELECT API_NAME, COUNT(*) AS Usage_Count " +
	                "            FROM DATA_USAGE_API " +
	                "            WHERE YEARWEEK(REQUEST_TIMESTAMP) = YEARWEEK(CURRENT_DATE) " +
	                "            GROUP BY API_NAME " +
	                "            ORDER BY Usage_Count DESC " +
	                "            LIMIT :top) top_10 ON du.API_NAME = top_10.API_NAME " +
	                "LEFT JOIN " + this.dbUtilsBilling.getSchemaName() + ".subscription s " +
	                ...
	                "WHERE (:owner::text IS NULL OR du.APPLICATION_OWNER = :owner) " +
	                "AND (:owner::text IS NULL OR s.is_active = true) " +
	                "AND EXTRACT(YEAR FROM du.REQUEST_TIMESTAMP) * 100 + EXTRACT(WEEK FROM du.REQUEST_TIMESTAMP) = " +
	        ...
	        break;
	    case "month":
	        query = "SELECT 'month' as type, TO_CHAR(du.REQUEST_TIMESTAMP, 'YYYY-MM-DD') AS intervalData, du.API_NAME, COUNT(*) AS total_usage " +
	                "FROM DATA_USAGE_API du " +
	                "INNER JOIN (SELECT API_NAME, COUNT(*) AS Usage_Count " +
	                "            FROM DATA_USAGE_API " +
	                "            WHERE EXTRACT(YEAR FROM REQUEST_TIMESTAMP) = EXTRACT(YEAR FROM CURRENT_DATE) " +
	        ...
	        break;
	    case "year":
	        query = "SELECT 'year' as type, TO_CHAR(du.REQUEST_TIMESTAMP, 'YYYY-MM') AS intervalData, du.API_NAME, COUNT(*) AS total_usage " +
	                "FROM DATA_USAGE_API du " +
	                "INNER JOIN (SELECT API_NAME, COUNT(*) AS Usage_Count " +
	                "            FROM DATA_USAGE_API " +
	                "            WHERE EXTRACT(YEAR FROM REQUEST_TIMESTAMP) = EXTRACT(YEAR FROM CURRENT_DATE) " +
	...
	public Page<LinkedHashMap<String, Object>> getFaultOvertimeDetails(String filter, String owner, int page,
		...
		switch (filter) {
	    case "today":
	        countQuery = "SELECT COUNT(DISTINCT CONCAT(API_NAME, '|', APPLICATION_NAME, '|', du.APPLICATION_OWNER)) " +
	            "FROM DATA_USAGE_API du " +
	            "LEFT JOIN " + this.dbUtilsUser.getSchemaName() + ".UM_USER uu ON du.APPLICATION_OWNER = uu.UM_USER_NAME " +
	            "LEFT JOIN " + this.dbUtilsUser.getSchemaName() + ".UM_USER_ATTRIBUTE attr ON uu.UM_ID = attr.UM_USER_ID AND attr.UM_ATTR_NAME = 'organizationName' " +
	            "LEFT JOIN " + this.dbUtilsBilling.getSchemaName() + ".subscription s ON s.subscription_id = du.SUBSCRIPTION_UUID " +
	            "WHERE (:owner::text IS NULL OR du.APPLICATION_OWNER = :owner) " +
	            "AND (:owner::text IS NULL OR s.is_active = true) " +
	            "AND du.APPLICATION_OWNER NOT IN ('anonymous','internal-key-app','UNKNOWN') " +
	...
	public List<LinkedHashMap<String, Object>> getFaultOvertime(String filter, String owner) throws Exception {
		...
		switch (filter) {
		case "today":
			query = "SELECT 'today' as type,TO_CHAR(du.REQUEST_TIMESTAMP, 'YYYY-MM-DD HH24') AS intervalData, du.API_NAME, COUNT(*) AS total_usage FROM DATA_USAGE_API du LEFT JOIN "
					+ this.dbUtilsBilling.getSchemaName()
					+ ".subscription s on s.subscription_id = du.SUBSCRIPTION_UUID WHERE (:owner::text IS NULL OR du.APPLICATION_OWNER = :owner) AND (:owner::text IS NULL OR s.is_active = true) AND du.APPLICATION_OWNER NOT IN ('anonymous','internal-key-app','UNKNOWN') AND NOT (du.PROXY_RESPONSE_CODE BETWEEN 200 AND 299) AND DATE(du.REQUEST_TIMESTAMP) = CURRENT_DATE GROUP BY intervalData, du.API_NAME ORDER BY intervalData DESC, total_usage DESC ";
			break;
		case "week":
			query = "SELECT 'week' as type,TO_CHAR(du.REQUEST_TIMESTAMP, 'YYYY-MM-DD') AS intervalData, du.API_NAME, COUNT(*) AS total_usage FROM DATA_USAGE_API du LEFT JOIN "
					+ this.dbUtilsBilling.getSchemaName()
					+ ".subscription s on s.subscription_id = du.SUBSCRIPTION_UUID WHERE (:owner::text IS NULL OR du.APPLICATION_OWNER = :owner) AND (:owner::text IS NULL OR s.is_active = true) AND du.APPLICATION_OWNER NOT IN ('anonymous','internal-key-app','UNKNOWN') AND NOT (du.PROXY_RESPONSE_CODE BETWEEN 200 AND 299) AND EXTRACT(YEAR FROM du.REQUEST_TIMESTAMP) * 100 + EXTRACT(WEEK FROM du.REQUEST_TIMESTAMP) = EXTRACT(YEAR FROM CURRENT_DATE) * 100 + EXTRACT(WEEK FROM CURRENT_DATE) GROUP BY intervalData, du.API_NAME ORDER BY intervalData DESC, total_usage DESC ";
		case "month":
			query = "SELECT 'month' as type,TO_CHAR(du.REQUEST_TIMESTAMP, 'YYYY-MM-DD') AS intervalData, du.API_NAME, COUNT(*) AS total_usage FROM DATA_USAGE_API du LEFT JOIN "
					+ this.dbUtilsBilling.getSchemaName()
					+ ".subscription s on s.subscription_id = du.SUBSCRIPTION_UUID WHERE (:owner::text IS NULL OR du.APPLICATION_OWNER = :owner) AND (:owner::text IS NULL OR s.is_active = true) AND du.APPLICATION_OWNER NOT IN ('anonymous','internal-key-app','UNKNOWN') AND NOT (du.PROXY_RESPONSE_CODE BETWEEN 200 AND 299) AND EXTRACT(YEAR FROM du.REQUEST_TIMESTAMP) = EXTRACT(YEAR FROM CURRENT_DATE) AND EXTRACT(MONTH FROM du.REQUEST_TIMESTAMP) = EXTRACT(MONTH FROM CURRENT_DATE)GROUP BY intervalData, du.API_NAME ORDER BY intervalData DESC, total_usage DESC ";
			break;
		case "year":
			query = "SELECT 'year' as type,TO_CHAR(du.REQUEST_TIMESTAMP, 'YYYY-MM') AS intervalData,du.API_NAME, COUNT(*) AS total_usage FROM DATA_USAGE_API du LEFT JOIN "
					+ this.dbUtilsBilling.getSchemaName()
					+ ".subscription s on s.subscription_id = du.SUBSCRIPTION_UUID WHERE (:owner::text IS NULL OR du.APPLICATION_OWNER = :owner) AND (:owner::text IS NULL OR s.is_active = true) AND du.APPLICATION_OWNER NOT IN ('anonymous','internal-key-app','UNKNOWN') AND NOT (du.PROXY_RESPONSE_CODE BETWEEN 200 AND 299) AND EXTRACT(YEAR FROM du.REQUEST_TIMESTAMP) = EXTRACT(YEAR FROM CURRENT_DATE) GROUP BY intervalData, du.API_NAME ORDER BY intervalData DESC, total_usage DESC ";
			break;
		default:
			throw new Exception("Invalid filter provided. Valid filters: today, week, month, year");
		}
	...
	public int getTotalAPIsByUsername(String username) {
		String query = "SELECT COUNT(*) AS totalAPI FROM AM_API WHERE (:username::text IS NULL OR AM_API.API_PROVIDER = :username) ";
	...
	public int getTotalUnpaidInvoicesByUsername(String username) {
		String query = "SELECT COUNT(i.id) as total FROM " + this.dbUtilsBilling.getSchemaName()
